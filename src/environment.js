...
const { readConfig, getPuppeteer } = require('jest-environment-puppeteer/lib/readConfig');
...
/**
 * This is a hell of an ugly hack. `PuppeteerEnvironment` should be extended
 * here, but `PuppeterEnvironment` extends `NodeEnvironment`. This project needs
 * `Puppeteer` environment to extend `JSDOMEnvironment`. To get around this,
 * the contents the class below have been copied here until a better solution
 * can be found.
 *
 * https://github.com/smooth-code/jest-puppeteer/blob/v4.4.0/packages/jest-environment-puppeteer/src/PuppeteerEnvironment.js#L16
 */
class JestPa11yEnvironment extends JSDOMEnvironment {
  ...
  async teardown() {
    const {
      page, context, browser, puppeteerConfig,
    } = this.global;
...
module.exports = JestPa11yEnvironment;
